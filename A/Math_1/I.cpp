...
bignum res;
void dfs(int u,int fn,int n,int cnt,bignum lcm,bignum m)
{
    if(u==fn)
    {
        if(cnt&1)res-=Pow(m/lcm,n);
        else res+=Pow(m/lcm,n);
    }
    else
    {
        dfs(u+1,fn,n,cnt,lcm,m);
        dfs(u+1,fn,n,cnt+1,lcm*f[u],m);
    }
}
