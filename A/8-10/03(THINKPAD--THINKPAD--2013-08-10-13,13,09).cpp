...
struct Segment_Tree
{
    ...
    void Push_Down(int rt)
    {
        ...
        if(npop[rt])
        {
            for(int i=0; i<op[rt].size(); ++i)
            {
                ...
                if(k==0)
                {
                    p[2][lc] = p[2][lc] + 3*_val*p[1][lc]%mod + 3*_val*_val*p[0][lc] + _val*_val%mod*_val*cnt[lc]%mod;
                    p[1][lc] = p[1][lc] + 2*_val*p[0][lc]%mod +_val*_val%mod*cnt[lc]%mod;
                    p[0][lc] = p[0][lc] + _val*cnt[lc]%mod;
                }
                else
                {
                ...
                if(k==0)
                {
                    p[2][rc] = p[2][rc] + 3*_val*p[1][rc]%mod + 3*_val*_val%mod*p[0][rc]%mod + _val*_val%mod*_val*cnt[rc]%mod;
                    p[1][rc] = p[1][rc] + 2*_val*p[0][rc]%mod +_val*_val%mod*cnt[rc]%mod;
                    p[0][rc] = p[0][rc] + _val*cnt[rc]%mod;
                }
                else
                {
    ...
    void Update_op(int L,int R,int rt,int from,int to,int _val,int k)
    {
        if(from<=L&&R<=to)
        {
            ...
            if(k==0)
            {
                p[2][rt] = p[2][rt] + 3*_val%mod*p[1][rt]%mod + 3*_val*_val%mod*p[0][rt]%mod + _val*_val%mod*_val*cnt[rt]%mod;
